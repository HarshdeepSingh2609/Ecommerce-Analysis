...
SELECT BRANCH,CATEGORY,RK FROM
(
SELECT BRANCH,CATEGORY,RATING,DENSE_RANK() OVER (PARTITION BY BRANCH ORDER BY RATING DESC ) AS RK 
FROM WALMART_PREPROCESSED
) AS X WHERE X.RK=1;
...
SELECT DATE_FORMAT(DATE,'%d/%m/%y')as day FROM WALMART_PREPROCESSED;
...
ORDER BY BRANCH ASC ,MONTHLY_REVENUE DESC ;
